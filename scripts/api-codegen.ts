...
function parseArgs(argv: string[]) {
  ...
  return out as { api?: string; out?: string; baseUrl?: string };
}
...
function sanitizeName(method: string, pattern: string): string {
...
function emitHeader(): string {
  return `// GENERATED by hipst scripts/api-codegen.ts on ${new Date().toISOString()}
...
async function main() {
  ...
  if (!args.api) {
    console.error('Usage: bun run scripts/api-codegen.ts --api <path>[#export] [--out <file|dir>] [--baseUrl <url>]');
    process.exit(1);
  }
  const [apiPathRaw, exportName] = String(args.api).split('#');
  ...
  if (!apiRoot) {
    console.error(`Could not resolve ApiComponent export '${exportName || 'default'}' from ${apiPath}`);
    process.exit(1);
  }
  const endpoints = buildTree(apiRoot);
  if (endpoints.length === 0) {
    console.warn('No handlers detected; nothing to generate.');
  }
  ...
  if (args.baseUrl) {
    // include a small helper wrapper with fixed baseUrl for convenience
  ...
  console.log('hipst api-codegen: wrote', outFile);
}
